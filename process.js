...
const RRESLT = /^\{(.+?) threads\} -> \{(.+?)ms, (.+?)ms mark, (.+?)ms init, (.+?) slots, (.+?) iters, (.+?) modularity\} (.+)/m;
...
function readLogLine(ln, data, state) {
  ...
  if (ROMPTH.test(ln)) {
    ...
    var [, num_threads, time, marking_time, initialization_time, num_slots, iterations, modularity, technique] = RRESLT.exec(ln);
    data.get(state.graph).push(Object.assign({}, state, {
      ...
      num_slots:   parseFloat(num_slots),
